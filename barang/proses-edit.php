...
if (isset($_POST["submit"])) {
    ...
    if (!empty($_FILES["image"]["name"]) && !empty($_POST["nama"]) && !empty($_POST["stok"]) && !empty($_POST["punya"]) && !empty($_POST["modal"]) && !empty($_POST["jual"]) && !empty($_POST["korting"]) && !empty($_POST["rak"]) && !empty($_POST["kategori"])) {
        ...
        if (in_array($fileType, $allowTypes)) {
            ...
            if ($compressedImage) {
                $sqlTambah = "UPDATE barang SET nama_barang='$namaBarang',stok='$stok',punya='$punya',harga_modal='$hargaModal',harga_jual='$hargaJual',harga_korting='$hargaKorting',foto='$fileName' ,lokasi='$lokasiBaru' WHERE id='$_POST[id]'";
                if ($konek->query($sqlTambah) === TRUE) {
?>
                    <script>
                        alert("Selamat, Gambar anda telah di-update");
        ...
        if ($konek->query($sqlUpdate) === TRUE) {
            ?>
            <script>
                alert("Selamat, Data anda telah di-update");
                window.location.href = ".././";
            </script>
        <?php
        } else {
        ...
        ?>
        <script>
            alert("Maaf silahkan isi data dengan benar :(");
            window.location.href = ".././";
        </script>
<?php
    }
